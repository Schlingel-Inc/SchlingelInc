    ...
    for i, dot in ipairs(frame.dots) do
        if i <= total then
            ...
            elseif i == currentStep then
                dot:SetColorTexture(unpack(SchlingelInc.Constants.FORM_COLORS.TITLE))  -- active: gold
            else
...
local function BuildFrame()
    ...
    divTop:SetColorTexture(unpack(SchlingelInc.Constants.FORM_COLORS.DIVIDER))
    ...
    divBot:SetColorTexture(unpack(SchlingelInc.Constants.FORM_COLORS.DIVIDER))
...
local function RenderDiscord(frame)
    ...
    eb:SetMaxLetters(SchlingelInc.Constants.DISCORD_HANDLE_MAX_LENGTH)
...
local function RenderProfessions(frame)
    ...
    for slot = 1, 2 do
        ...
        skillEb:SetBackdropColor(unpack(SchlingelInc.Constants.FORM_COLORS.FORM_BG))
        skillEb:SetBackdropBorderColor(unpack(SchlingelInc.Constants.FORM_COLORS.FORM_BORDER))
