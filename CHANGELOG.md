...
- Vorgefertigte Erfolge: Alle Level-Meilensteine (10/20/25/30/40/50/60), jeweils auch als "ohne zu sterben"-Variante, sind ab Werk im Erfolge-Katalog vorhanden und werden auch rückwirkend für bereits high-level Charaktere freigeschaltet. Die Katalog-Einträge dieser eingebauten Erfolge (Name, Punkte, Kriterien) können von Offizieren nicht verändert oder eingestellt werden.
- Erfolge können von Offizieren als "Global" markiert werden – der Fortschritt gilt dann für den ganzen Account statt nur für den einzelnen Charakter.
- Offiziere können jetzt auch Level-Erfolge manuell an einzelne Spieler verleihen, nicht nur RP-Erfolge – etwa um einen strittigen Death-Counter-Fall zu klären (Kill-Zähler-Erfolge bleiben weiterhin ausschließlich automatisch). Das "Erfolg verleihen"-Fenster fragt dafür beim Zielspieler live ab, welche Erfolge noch nicht erreicht wurden, und zeigt nur diese zur Auswahl an; kommt keine Antwort, erscheint ein Hinweis, dass der Status nicht bestätigt werden konnte.
- Neue Option "Popup-Edit-Modus" (Optionen-Menü): zeigt Platzhalter für alle Popup-Fenster an, damit deren Positionen unabhängig vom eigentlichen Auslöser gesetzt und gespeichert werden können.
- Bugfix: Enter-Taste in den Popups "Deathcounter setzen" und "Schande verhängen" konnte abstürzen, wenn kein passender Bestätigen-Button gefunden wurde.
- Neuer "Raid"-Tab im Gildenpanel (Raid Panel mit LFG-light): Mitglieder können Raids posten (Titel, Instanz, Datum/Uhrzeit, optionale Notiz) und per Klick mit Rolle zusagen. Einträge werden gildenweit synchronisiert und laufen automatisch ab.
